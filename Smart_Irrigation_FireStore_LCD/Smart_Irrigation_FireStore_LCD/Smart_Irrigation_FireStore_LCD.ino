...
const char *ssid = "T(-E-)G"; // Wifi network name.
const char *pass = "*T1972023-E-1SMT'";
...
const int rs = 25, en = 33, d4 = 12, d5 = 14, d6 = 27, d7 = 26, rw = 32; // Define RW pin
LiquidCrystal lcd(rs, rw, en, d4, d5, d6, d7); // Initialize LCD with RW pin
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    displaySensorReadingsOnLCD(); //(double mappedSoilMoisture, double waterLevel);
  }
}

void displaySensorReadingsOnLCD() {
  lcd.clear();
  ...
  lcd.setCursor(0, 1); // تحريك المؤشر للصف الثاني
  lcd.print("Humidity: ");
  lcd.print(dht.readHumidity());
  lcd.print(" %");
  delay(2500);

  lcd.setCursor(0, 0); // تحريك المؤشر للصف الأول مرة أخرى
  lcd.print("Soil: ");
  lcd.print(mappedSoilMoisture);
  lcd.print(" %");
  delay(2500);

  lcd.setCursor(0, 1); // تحريك المؤشر للصف الثاني مرة أخرى
  lcd.print("Wat.Lev: ");
...
void updateFirebase(float temperature, float humidity) {
  ...
  content.set("fields/ledPin/integerValue", ledPin);
  content.set("fields/pumpState/integerValue", pumpState);
